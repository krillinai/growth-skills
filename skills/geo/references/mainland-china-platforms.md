...
Use ordinary permitted product access only. A manual record may establish exactly what its retained artifact shows: the product, surface, displayed model or `unexposed`, exposure, displayed search/network state or `unexposed`, login state, market, language, timestamp, repetition, access state, answer state, named-entity mention, inspectable citation/source URLs, and error.
