...
| MEA-01 | For every page, site, visibility, or incident audit: an attributable panel, change artifact, or measurement record complete under the [canonical panel schema](visibility-observation.md#evidence-and-panel-schema), including its attempt records, planned and actual repetitions, outcomes, counts, denominators, and complete-panel metrics; when a comparison or change is claimed, both records must also contain the [canonical comparison match key](visibility-observation.md#canonical-comparison-match-key). | Every required measurement artifact and canonical panel field is present, attributable, inspectable, and internally consistent; a claimed comparison satisfies every canonical match-key rule. | Supplied measurement evidence verifies an invalid schema field, denominator, attribution, repetition record, panel metric, or canonical match-key comparison. If no panel, change, or measurement evidence is supplied, MEA-01 is unavailable, not fail or N/A. | Only when the entire request is routed out of GEO audit scope. MEA-01 is applicable to every in-scope page, site, visibility, and incident audit. |
