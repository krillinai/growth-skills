...
**Google Play boundaries.** Use Google Play terms: title, short description, full description, feature graphic, screenshots, and promotional video. Do not rename the short description an Apple subtitle. In an Apple audit, these Google-only fields are `not applicable`; in a Google Play audit, unavailable console or dynamically failed public components are `unavailable`, not failures. A loading skeleton, HTTP error, or non-rendered container proves only the stated failed observation. Request a static capture or supplied export and do not generalize copy from another locale or country.
