...
**Creative-focused work.** When scope is limited to named screenshots, assess only the relevant creative and localization checks. Record viewport, rendered asset size, message size, contrast, obstruction, and sequence as supplied or observed. For a collection-level screenshot property such as `APL-CRT-03`, define the screenshot set from the dated capture or supplied asset inventory, then record each member under the parent check. If membership is unknown or any applicable screenshot is unavailable or unrendered, mark the parent check unavailable and score no partial set; screenshots do not receive their own points or check rows. Link every adverse observed screenshot to its finding. Label an expected conversion consequence `inferred`; performance remains unavailable unless measured.
