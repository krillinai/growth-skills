...
**Apple boundaries.** The keyword field is not publicly observable. `APL-MET-03` promotional text and `APL-CRT-04` app preview are conditional checks. When a complete, dated product-page capture verifies either component is absent, mark that check `not applicable` and exclude its 3 or 4 points only when the declared scope does not require that surface. When a failed or partial render cannot establish whether the component is present, mark the check `unavailable`. When the declared listing/scope makes that component's presence or quality applicable, retain the check: verified absence is `fail` (0/max and a finding), not `not applicable`. Do not call the subtitle a Google Play short description, and mark Google Play-only fields such as feature graphic and short description `not applicable` in an Apple assessment. Do not assert a screenshot-count policy or compliance without a current, dated Apple primary source and matching product-page evidence.
