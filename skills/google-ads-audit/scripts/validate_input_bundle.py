#!/usr/bin/env python3
"""Validate a canonical local input bundle for Google Ads Audit."""
...
CSV_SCHEMAS = {
    ...
    "conversion-actions.csv": {
        ...
        "recommended": {
            ...
            "value_setting",
            "attribution_model",
            "click_through_window_days",
            "view_through_window_days",
            "include_in_conversions",
            "last_changed_at",
        },
        "unique": "conversion_action_id",
    },
    "daily-performance.csv": {
        ...
        "recommended": {
            ...
            "engaged_view_conversions",
            "invalid_clicks",
            "currency",
            "conversion_action_scope",
        },
    },
    "ad-groups.csv": {
        "required": {
    ...
    "ads.csv": {
        ...
        "recommended": {"destination_url", "concept_id", "offer", "proof", "cta"},
        "unique": "ad_id",
    },
    "keywords.csv": {
        "required": {
        ...
        "recommended": {"final_url", "quality_score", "first_page_cpc", "negative"},
        "unique": "keyword_id",
    },
    "search-terms.csv": {
        "required": {
    ...
    "assets.csv": {
        "required": {"asset_id", "campaign_id", "asset_type", "concept_id", "status"},
        "recommended": {
            "ad_group_id",
            "asset_group_id",
            "asset_name",
    ...
    "landing-pages.csv": {
        "required": {"landing_page_id", "campaign_id", "url", "captured_at"},
        "recommended": {
            "ad_group_id",
            "ad_id",
            "market",
...
REQUIRED_FILES = {"context.json", "campaigns.csv", "conversion-actions.csv"}
OPTIONAL_FILES = set(CSV_SCHEMAS) - {"campaigns.csv", "conversion-actions.csv"}
...
FORBIDDEN_HEADER_PATTERNS = (
    ...
    re.compile(r"(^|_)(access_token|refresh_token|developer_token|api_key|client_secret|password)($|_)"),
    re.compile(r"(^|_)(gclid|gbraid|wbraid|customer_match)($|_)"),
)

DATE_COLUMNS = {"date", "captured_at", "start_date", "end_date", "last_changed_at"}
NUMERIC_COLUMNS = {
    "budget_amount",
    "optimization_score",
    "click_through_window_days",
...
BOOLEAN_COLUMNS = {"primary_for_goal", "include_in_conversions", "negative"}
...
def read_csv(path, filename, errors, warnings):
    ...
    if forbidden:
        errors.append(
            f"{filename}: forbidden identifier or credential header(s): {', '.join(forbidden)}"
        )
    if not rows:
...
def validate_context(path, errors):
    ...
    for forbidden_path in find_forbidden_keys(context):
        errors.append(f"context.json: forbidden identifier or credential field: {forbidden_path}")
...
def require_parent(rows_by_file, child_file, child_field, parent_file, parent_field, errors):
    ...
    for row_number, row in enumerate(rows_by_file.get(child_file, []), start=2):
        ...
        if value and value not in parents:
            errors.append(
                f"{child_file}:{row_number}: {child_field} {value!r} is absent from {parent_file}"
            )


def validate_references(rows_by_file, errors):
    for child_file in CSV_SCHEMAS:
        if child_file in {"campaigns.csv", "conversion-actions.csv", "business-outcomes.csv"}:
            continue
        require_parent(rows_by_file, child_file, "campaign_id", "campaigns.csv", "campaign_id", errors)
    for child_file in ("ads.csv", "keywords.csv", "search-terms.csv", "assets.csv", "daily-performance.csv", "landing-pages.csv"):
        require_parent(rows_by_file, child_file, "ad_group_id", "ad-groups.csv", "ad_group_id", errors)
    for child_file in ("daily-performance.csv", "landing-pages.csv"):
...
def print_text_result(result):
    print(f"Google Ads audit input bundle: {'VALID' if result['valid'] else 'INVALID'}")
...
def main(argv=None):
    parser = argparse.ArgumentParser(description="Validate a canonical Google Ads audit input bundle.")
